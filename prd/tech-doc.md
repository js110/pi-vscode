...
| 面板放置标记 | globalState `pi-agent.sidebarPlacementDone` | 无（新增） | **已移除**（2026-09-16）：首次运行移动 hack 删除；容器位置最终声明为 `viewsContainers.activitybar`（面板默认左侧，见 §2.10 e)），残留在用户 globalState 中的旧键无人读取、无害 | 旧版不读 |
...
**d) 状态栏常驻入口（2026-09-16）**：c) 的直接副作用——图标跟容器走，`secondarySidebar` 容器在活动栏没有图标，用户报告"左侧图标没看到"。稳定 API 无"图标在左 + 面板默认在右"形态（Copilot Chat 的形态由核心特例实现，扩展不可复制；容器放回 activitybar 则面板默认回左侧，拖到右侧后空容器图标同样消失）。用户确认采用状态栏入口弥补可见性：`StatusBarManager` 增加常驻 `$(comment-discussion)` 条目（priority 101，紧邻既有模型条目 100），点击执行既有 `pi-agent.focusChat`（与 Ctrl+Shift+L 同命令），会话初始化之前即可见；tooltip 走 i18n `statusBar.openPanel`，`pi-agent.displayLanguage` 变更时经 `refresh()` 更新。**保留**：容器回退 activitybar（见 e)）后该条目与位置无关，作为任意位置快速打开面板的入口继续存在。

**e) 面板位置最终回退为 activitybar 左侧（2026-09-16，用户决定"恢复到之前就放到左边的那版"）**：`viewsContainers` 从 `secondarySidebar` 改回 `activitybar`（id/title/icon 不变），面板默认回到主侧边栏左侧、活动栏恢复图标。同时删除 `moveSidebarView()` 与 `pi-agent.moveToSecondarySidebar`/`moveToPrimarySidebar` 两个手动命令——对本机 1.112 workbench bundle 的取证证实其依赖的 `workbench.action.moveViewToSecondarySideBar`/`moveViewToPrimarySideBar` **在 bundle 中不存在**（全文字面量 grep 为空），两个命令自始至终是必然失败的无操作（这正是旧版"首次运行自动移右"从未生效的根因：executeCommand reject 被 catch 写日志，而标记早已落盘）。1.112 真实存在的移动命令：`workbench.action.moveView`/`moveFocusedView`（交互式快选，不可脚本化传参）、`movePanelToSidePanel`/`movePanelToSecondarySideBar`/`moveSidePanelToPanel`/`moveSecondarySideBarToPanel`/`moveSideBarRight`/`moveSideBarLeft`（面板级）；无 `moveViewToPrimarySideBar`。另证实核心对扩展贡献的视图容器硬编码 `hideIfEmpty: true`（registerCustomViewContainer），视图被移走后容器即被活动栏隐藏——即使移动成功，"图标在左 + 面板在右"也无法共存；内部命令 `vscode.moveViews` 虽存在但未文档化，继续不采用（坚持 100% 稳定 API）。

### 2.11 内置斜杠命令分发（2026-09-17，用户报告"/compact 失败被当普通文本发给模型"）

**根因（SDK dist 取证）**：SDK `AgentSession.prompt()` 只负责三类斜杠输入——扩展注册命令（`pi.registerCommand`，流式中也可立即执行）、`/skill:name` 技能展开、文件型 prompt 模板展开（agent-session.d.ts:153-164, 355-388）；**23 个内置命令（`BUILTIN_SLASH_COMMANDS`，dist/core/slash-commands.js，含 quit 的模板字符串描述）是 Pi CLI 壳的职责**，SDK 不处理。扩展壳此前只有斜杠补全菜单（webview 纯 UI，session.ts `getCommands()` 镜像内置名单仅供展示），prompt/queueMessage 分发路径从未实现命令拦截——`/compact` 因此原样进入模型上下文（README 承诺的 "/login in the chat input" 同因从未生效）。

**方案（100% 稳定 API）**：

1. **纯模块** `shared/slash-commands.ts`：`BUILTIN_COMMANDS`（23 项名单+描述的精确镜像，session.ts `getCommands()` 改为从此导入，消除双份维护）；`isBuiltinSlashCommandName(name)`；`parseBuiltinSlashCommand(text)`——trim 后须以 `/` 开头、首 token 精确命中内置名单（大小写不敏感）才返回 `{name, args}`，其余一律 null（未知 `/foo` 继续走 SDK prompt() 的扩展命令/技能/模板路径，与 CLI 行为一致）；`extractLastAssistantText(messages)`——从尾向前找第一条带文本的 assistant 消息（跳过纯工具轮，多 text 段以 `\n` 连接）。
2. **host 拦截**：TabManager hooks 增 `writeClipboard`；`_maybeExecuteBuiltinCommand(tab, text)` 在 `prompt`、`queueMessage`、`steer` 三个 case 顶部调用（queueMessage/steer 拦截防"流式中斜杠命令被排队、稍后以普通文本入模"）；`editQueuedMessage` 拒绝把队列项改成内置命令（队列由 SDK 侧 drain 绕过 host，改完即走私）；protocol prompt 变体增 `bypassSlashCommands?: boolean`——inline-chat 与宿主入口（sendToPi）等程序化 prompt 豁免拦截（其 instruction 位于文本首部，输入 `/new` 会重置会话）；T6 的 `compactionAccept` 分支抽出 `_runManualCompaction(tab, customInstructions?)` 复用（compactionInFlight 守卫、清 compactionPrompt、messagesDirty、compactionResult 推送保留，成功路径补 compact.done toast）。
3. **映射（23 项全收口）**：已实现 10 项——`/compact [说明]` → SDK 原生 `session.compact(说明)`；`/new` → newSession；`/model` → selectModel；`/thinking [级别]` → 带参直接 setThinkingLevel（校验可用级别），无参 cycleThinkingLevel；`/name <名>` → 直接改名（不走 renameSession 分发，防会话列表面板弹出；无参提示用法）；`/resume` → getSessions（拉起会话列表）；`/settings`、`/login` → 打开设置面板（/login 附提示，兑现 README 承诺）；`/copy` → extractLastAssistantText → 剪贴板（空回复提示）；`/session` → 会话名/模型/上下文用量/文件路径信息提示。其余 13 项（tree/scoped-models/export/import/share/changelog/hotkeys/fork/clone/trust/logout/reload/quit）→ 明确提示"面板暂不可用"（不再静默入模）。
4. **流式与锁语义**：改轮状态的 /compact、/new 在流式中拒绝（`slash.blockedStreaming`）；单写者锁激活（occupancy !== 'none'）时 /compact、/new、/name 只读拒绝（AC-OP-03）；/compact 在 compactionInFlight 时提示进行中；其余命令与流式/锁无关，照常执行。
5. **审查修复（general-purpose 审查：1H+3M+6L，无 CRITICAL）**：H-1 选区芯片豁免——sidebar 的自动 attach 包裹条件加 `!parseBuiltinSlashCommand(msg.text)`（否则前置选区前言使 host 拦截失效且芯片被烧在命令消息上）；M-1 锁守卫；M-2 SDK dist 名单守卫测试——读 `node_modules/@earendil-works/pi-coding-agent/dist/core/slash-commands.js` 提取 `name: "..."` 字面量与镜像集合全等断言（skipIf 文件缺失），SDK 升级时强制同步名单；M-3 steer 拦截；L-3 bypassSlashCommands 协议字段；L-5e getCommands() 扩展命令去重——与内置同名的扩展注册命令不再展示（host 拦截使其不可达）。
6. i18n 增 `slash.*` 7 键（EN/ZH 插值参数一致，守卫测试覆盖）。测试：slash-commands.test.ts 14 例（解析/参数/大小写/未知回落/技能不误判/最后 assistant 文本提取 13 例 + SDK dist 名单守卫 1 例）+ tab.test.ts 增 22 例（compact 带/不带参/流式拒绝/锁拒绝/inFlight 提示、new/model/thinking 带参/name/resume/settings/login/copy/空 copy/unsupported/未知回落、流式中 queue 拦截与普通排队、queueMessage /compact+/new 拒绝、steer 拦截与放行、editQueuedMessage 拒改斜杠命令、/name 不弹会话列表）。全量 501 passed / 5 skipped。
7. **后续补遗（用户实测反馈）**：① 补全菜单排序——原实现技能恒排命令前、名称匹配与描述匹配同权重，输入 `/compa` 时描述里碰巧含 "Compact" 的 `/skill:handoff` 压过名称前缀命中的 `/compact`；改 `rankSlashMenuItems`（slash-commands.ts 纯函数）：名称/label 前缀(0) > 名称/label 子串(1) > 描述(2)，无命中剔除、空查询保序、同分保注册序，`/skill:` 前缀输入经 label（去 `/`）前缀匹配。② 压缩错误分类——SDK `keepRecentTokens` 默认 20000，小会话（如 7.3k tokens）`prepareCompaction` 返回 undefined 抛 "Nothing to compact (session too small)"，系正常"无需压缩"而非失败，但原 catch 吞错显示通用失败文案误导用户；compactionResult 协议加 `message?`，host 按 "Nothing to compact|session too small|Already compacted" 分类给 `compact.nothingToCompact` 文案，其余错误附 humanize 详情，webview 优先显示 message（不再双条误导）。
