...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    async newSession(): Promise<void> {
        await this._handleMessage({ type: 'newSession' });
    }

    async abort(): Promise<void> {
        await this._activeTab.session.abort();
    }

    async selectModel(): Promise<void> {
        await this._activeTab.session.showModelPicker();
        this.sendStateSync();
    }

    toggleThinking(): string | undefined {
        const level = this._activeTab.session.cycleThinkingLevel();
        this.sendStateSync();
        return level;
    }

    async compact(): Promise<void> {
        await this._activeTab.session.compact();
        this.sendStateSync();
    }
