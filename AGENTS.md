...
- **No direct DOM libraries**: The webview UI is built with vanilla TypeScript and DOM APIs. No React, no framework. Rendering uses `el()`/`escHtml()` from `src/webview/dom.ts`. Pure, state-free DOM builders live in `src/webview/render/messages.ts` (happy-dom-testable); keep them free of `state`, `vscode`, and side effects. The streaming region is the deliberate exception: its reconcilers (`updateStreamingThinking`/`reconcileStreamingText`) mutate an already-mounted node in place and keep a per-element WeakMap block cache, so the markdown body isn't rebuilt every frame. `main.ts` is the orchestrator wiring events and live state.
